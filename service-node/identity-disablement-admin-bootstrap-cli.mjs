import { bootstrapIdentityDisablementAdministrator } from "./identity-state-admin-bootstrap.mjs";
...
async function main() {
  ...
  try {
    ...
    const result = await bootstrapIdentityDisablementAdministrator(repository, {
      database: databaseConfig.database,
      allowBootstrap: process.env.N09_ALLOW_IDENTITY_DISABLEMENT_BOOTSTRAP,
    ...
    console.log(JSON.stringify({
      event: "identity_disablement_bootstrap_completed", database: databaseConfig.database,
...
main().catch(() => {
  console.error(JSON.stringify({ event: "identity_disablement_bootstrap_failed" }));
  process.exitCode = 1;
});
