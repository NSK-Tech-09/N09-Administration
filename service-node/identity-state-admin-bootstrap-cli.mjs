...
async function main() {
  ...
  try {
    ...
    const result = await bootstrapIdentitySuspensionAdministrator(repository, {
      database: databaseConfig.database,
      allowBootstrap: process.env.N09_ALLOW_IDENTITY_STATE_BOOTSTRAP,
    ...
    console.log(JSON.stringify({
      event: "identity_state_bootstrap_completed", database: databaseConfig.database,
...
main().catch(() => {
  console.error(JSON.stringify({ event: "identity_state_bootstrap_failed" }));
  process.exitCode = 1;
});
