import { bootstrapIdentityReactivationAdministrator } from "./identity-state-admin-bootstrap.mjs";
...
async function main() {
  ...
  try {
    ...
    const result = await bootstrapIdentityReactivationAdministrator(repository, {
      database: databaseConfig.database,
      allowBootstrap: process.env.N09_ALLOW_IDENTITY_REACTIVATION_BOOTSTRAP,
    ...
    console.log(JSON.stringify({
      event: "identity_reactivation_bootstrap_completed", database: databaseConfig.database,
      identity_id: process.env.N09_ADMIN_IDENTITY_ID, created: result.created,
...
main().catch(() => {
  console.error(JSON.stringify({ event: "identity_reactivation_bootstrap_failed" }));
  process.exitCode = 1;
});
