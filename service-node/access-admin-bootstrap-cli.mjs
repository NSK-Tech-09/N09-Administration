...
async function main() {
  ...
  try {
    ...
    const result = await bootstrapAccessAdministrator(repository, {
      database: databaseConfig.database,
      allowBootstrap: process.env.N09_ALLOW_ACCESS_DIRECTORY_BOOTSTRAP,
    ...
    console.log(JSON.stringify({
      event: "access_administrator_bootstrap_completed", database: databaseConfig.database,
...
main().catch(() => {
  console.error(JSON.stringify({ event: "access_administrator_bootstrap_failed" }));
  process.exitCode = 1;
});
