...
async function main() {
  ...
  try {
    ...
    const result = await bootstrapIdentityLinkAdministrator(repository, {
      database: databaseConfig.database,
      allowBootstrap: process.env.N09_ALLOW_IDENTITY_LINK_ADMIN_BOOTSTRAP,
    ...
    console.log(JSON.stringify({
      event: "identity_link_administrator_bootstrap_completed", database: databaseConfig.database,
...
main().catch(() => {
  console.error(JSON.stringify({ event: "identity_link_administrator_bootstrap_failed" }));
  process.exitCode = 1;
});
