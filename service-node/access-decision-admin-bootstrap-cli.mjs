...
async function main() {
  ...
  try {
    ...
    const result = await bootstrapAccessDecisionAdministrator(repository, {
      database: databaseConfig.database,
      allowBootstrap: process.env.N09_ALLOW_ACCESS_DECISION_BOOTSTRAP,
    ...
    console.log(JSON.stringify({
      event: "access_decision_administrator_bootstrap_completed",
      database: databaseConfig.database,
...
main().catch(() => {
  console.error(JSON.stringify({ event: "access_decision_administrator_bootstrap_failed" }));
  process.exitCode = 1;
});
